...
use crate::sql::{request_logs, response_logs, proxy_stats, proxy_stats_hourly, domain_blocks, domain_pattern_blocks};
...
async fn set_all_partitions(conn: &deadpool_postgres::Object, config: &DbConfig) -> Result<()> {
    ...
    match create_partitions(conn, TableType::ProxyStatsHourly, today, future_partitions + 1).await {
